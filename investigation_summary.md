...
| 4 | `atlauren-wa` in `Protected Users` (hard NTLM block at KDC) | `MemberOf -match 'Protected Users'` | **Ruled out** — not a member |
...
| 7 | Account/endpoint fundamentally broken (rights, environment) | Action4 (Scheduled Task) actually renamed the computer successfully | **Ruled out** — `atlauren-wa` has all necessary rights; the endpoint works fine via a different code path |
...
1. **Differential test never run**: retry `override runas=localuser` (Action1 or Action2) with a *different* domain account on the same endpoint — would confirm whether it's specific to `atlauren-wa` (unlikely at this point, given how much has been ruled out) or universal to this endpoint's BES Client.
